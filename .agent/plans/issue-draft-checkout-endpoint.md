...
## Acceptance Criteria
- [x] `POST /api/v1/bookings/:bookingId/quote` (customer-only, not party-based — CodeRabbit flagged the initial party-based `isBookingParty` gate as too permissive for a charge-initiating endpoint; fixed to `session.userId === booking.customerUserId`, so the assigned provider can never trigger a capture themselves. Unlike the read-only sibling endpoints, which stay party-based on purpose. Booking must be `status === 'accepted'` — 409 otherwise): idempotently returns an existing non-expired quote for this booking if one exists, otherwise creates and returns a new one. Response: `{ quoteId, bookingId, lineItems, calloutFeeCents, laborCents, platformFeeCents, totalCents, currency: 'EUR', pricingTableVersion, createdAt, expiresAt }`. Computed via `computeBookingPrice(booking.serviceCategory, booking.urgency)` from slice 1 — the client supplies nothing pricing-related.
...
- [x] New in-memory-only repository (mirrors `payment-methods`' shape, not `payouts`/`invoices` — no Postgres implementation needed this slice): `bookings/domain/quote.repository.ts` (`QuoteRecord`, `CreateQuoteInput`, `QUOTE_REPOSITORY` symbol, `createQuote`, `getQuoteById`, `getOrCreateActiveQuote`), `bookings/infrastructure/in-memory-quote.repository.ts`. CodeRabbit flagged the original two-call `getActiveQuoteForBooking` + `createQuote` shape as a race (two concurrent requests for the same booking could both observe "no active quote" and each create one); replaced with a single atomic `getOrCreateActiveQuote` with no `await` between the check and the write. Note this was never a payment-safety bug — `capturePaymentForBooking` stays idempotent by `bookingId` regardless of which duplicate `quoteId` reached checkout — only quote-store hygiene, but worth closing since the fix was small. Lives inside `bookings/` (not a new top-level module) because quote/checkout are booking-scoped actions exactly like the existing `getBookingTracking`/`getBookingProviderIdentity`/`getBookingContact` methods already on `BookingsService` — not a new cross-cutting domain like `payments`/`payouts`.
